...
def fight(participant1, participant2, first2attack):
    ...
    while variable1 > 0 and variable2 > 0:
        if first2attack==1:
            attackmult = attack_multiplier(participant1[1],participant2[1])
            damage = attackmult * participant1[3]
            variable2 = variable2-damage
            first2attack=2
        else:
            attackmul = attack_multiplier(participant2[1],participant1[1])
            dmg = attackmul * participant2[3]
            variable1 = variable1-dmg
            first2attack=1
        rounds = rounds + 1
    if variable1 >0:
        winner=1
...
def tournament(participants):
    ...
    for i in participants:
        x=participants.index(i)
        for p in participants:
            t=participants.index(p)
            if i!=p:
                results = fight(i,p,1)
                if results[0]==1:
                    wins[x] += 1
