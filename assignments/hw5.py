...
def calculateDensity(fcpolygon, attribute,geodatabase = "assignment2.gdb"):
    ...
    if desc.Name == "WGS84" or "NAD83":
        print("Reproject polygon to coordinate system that can more acturately calculate area.")
        ...
        arcpy.CalculateGeometryAttributes_management(fcpolygon, [["area_sqmi", "AREA_GEODESIC" ]], "MILES_US")
        ...
        arcpy.management.CalculateField(fcpolygon, "Density", attribute/"area_sqmi", "SQL")
...
def estimateTotalLineLengthInPolygons(fcLine, fcClipPolygon, polygonIDFieldName, clipPolygonID, geodatabase = "assignment2.gdb"):
    #import arcpy and or and then set overwriteOutput to true
    import arcpy
    import os
    arcpy.env.overwriteOutput = True
    #set the workspace to be the geodatabase provided
    arcpy.env.workspace = geodatabase
    #perform select layer by attribute to select the spefic polygon that you will use to clip the polyline feature class with
    clipFC = arcpy.management.SelectLayerByAttribute(fcClipPolygon, "NEW_SELECTION", polygonIDFieldName = clipPolygonID)
    #preform clip analysis to clip out the polyline within the clipFC polygon
    arcpy.analysis.Clip(fcLine, clipFC, "clippedFC")
    #set a variable length to be equal to zero, we will add the shape length onto this variable to find the length
    length = 0
    #this is a search cursor that pulls the shape length of all the lines within the polygon and adds them together and then prints out the total length
    with arcpy.da.SearchCursor("clippedFC", "Shape_Length") as cursor:
        for row in cursor:
            for i in row:
                length += i
    print(length)
...
def countObservationsWithinDistance(fcPoint, distance, distanceUnit, geodatabase = "assignment2.gdb"):
    #import arcpy os and set overwrite to true
    import arcpy
    import os
    arcpy.env.overwriteOutput = True
    #set workspace to be the geodatabase
    arcpy.env.workspace = geodatabase
    #use PointDistance analysis to find the number of points within the specified distance of the point
    arcpy.analysis.PointDistance(fcPoint, fcPoint, "OutputTable", distance distanceUnit)
    #use the AddJoin tool to join the output table back to the point table
    arcpy.management.AddJoin(fcPoint, "OBJECTID_1", "OutputTable", "INPUT_FID")
